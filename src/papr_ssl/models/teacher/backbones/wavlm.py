"""Raw-waveform WavLM frame-representation wrapper."""
...
class WavLMBackbone(nn.Module):
    """Select one exact `outputs.hidden_states` index from WavLM."""
    ...
    def _load_pretrained_model(self) -> nn.Module:
        try:
            ...
            raise ImportError(
                "transformers is required to load a real WavLM checkpoint"
            ) from exc
    ...
    def train(self, mode: bool = True) -> "WavLMBackbone":
        super().train(mode)
        if self.freeze_backbone:
            self.model.eval()
        return self
