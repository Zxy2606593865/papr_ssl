#!/usr/bin/env python
"""P4-04: run one W2v-BERT 2.0 hidden_state_index x seed condition."""
...
def main() -> int:
    ...
    p.add_argument(
        "--base-teacher-config",
        type=Path,
        default=Path("configs/p3/teacher/w2v_bert2.yaml"),
    )
    p.add_argument(
        "--cache-root",
        type=Path,
    ...
    p.add_argument(
        "--sweep-root",
        type=Path,
        default=Path("artifacts/p4_runs/w2v_bert2"),
    )
    p.add_argument(
        "--device",
    ...
    if not (cache_dir / "manifest.json").is_file():
        raise FileNotFoundError(
            f"missing P4-04 cache for hidden_states[{layer}]: {cache_dir}"
        )
    ...
    write_json(
        ...
        {
            "schema": "papr_ssl.p4_04_w2vbert2_run_result.v1",
            "phase": "P4-04",
            "backbone": "w2v_bert2",
            "hidden_state_index": layer,
            "seed": seed,
    ...
    print("=" * 108)
    print(f"P4-04 W2V-BERT 2.0 hidden_states[{layer}] seed={seed}")
    print("=" * 108)
    ...
    print("-" * 108)
    print("P4-04 SINGLE RUN: COMPLETE")
    return 0
