#!/usr/bin/env python
"""P4-02: run one Wav2Vec2 hidden_state_index x seed condition."""
...
def main() -> int:
    ...
    if not (cache_dir / "manifest.json").is_file():
        raise FileNotFoundError(
            f"missing P4-02 cache for hidden_states[{layer}]: {cache_dir}"
        )
    ...
    write_json(
        ...
        {
            "schema": "papr_ssl.p4_02_wav2vec2_run_result.v1",
            "phase": "P4-02",
    ...
    print("=" * 104)
    print(
        f"P4-02 WAV2VEC2 hidden_states[{layer}] seed={seed}"
    )
    print("=" * 104)
    ...
    print("-" * 104)
    print("P4-02 SINGLE RUN: COMPLETE")
    return 0
